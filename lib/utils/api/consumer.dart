...
class Consumer {
  ...
  generateQuery() {
    ...
    orders?.forEach((key, value) {
      _temp.add('$key[sort]=$value');
      // _tempOrder += '$key[sort]=$value';
    });

    fillters?.forEach((key, value) {
      _temp.add(_applyQueryFilter(key, value));
      // _tempFillter += _applyQueryFilter(key, value);
    });

    joins?.map((e) {
      _temp.add(_applyQueryFilter('with', e));
      // _tempJoin += _applyQueryFilter('with', e);
    });

    if (limits != null) {
      _temp.add('&limit=$limits');
    }
    // String _tempLimit = limits != null ? '&limit=$limits' : '';

    // String _combineFillter = _tempFillter + _tempJoin + _tempOrder + _tempLimit;
    UtilLogger.log('COMBINE FILTER', _temp);
    return '';
    // _combineFillter =
